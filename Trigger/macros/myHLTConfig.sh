...
hltGetConfiguration /users/musella/dipho0T/V4 --full --offline --mc --unprescale --process myHLT \
...
sed 's%process = cms.Process( "myHLT" )%process = cms.Process( "myHLT" )\nprocess.load("diphotons/Trigger/setup_cff")%; s%_customInfo.*maxEvents.*=.*%_customInfo["maxEvents"]=-1%' -i hlt.py

sed 's%from HLTrigger.Configuration.customizeHLTforALL import customizeHLTforAll%fromfrom diphotons.Trigger.JobConfig import customize
try:
...
from HLTrigger.Configuration.customizeHLTforALL import customizeHLTforAll%' -i hlt.py
