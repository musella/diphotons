...
declare -A eras=( ["RunB"]="run < 275654"
                  ["RunC"]="run >= 275654 && run < 276315"
                  ["RunD"]="run >= 276315 && run < 276827"
                  ["RunE"]="run >= 276827 && run < 277851"
                  ["RunF"]="run >= 277851 && run < 278815"
                  ["RunG"]="run >= 278815 && run < 281613"
                  ["RunH"]="run >= 281613"
                )

for era in RunB RunC RunD RunE RunF RunG;
do
    echo "${eras[$era]}"
    ./auto_plotter.py --selection cic \
                  --file  /afs/cern.ch/user/s/spigazzi/work/Xgg/CMSSW_8_0_20/src/diphotons/Analysis/config/double_ele_fall16_v1/output.root --move cicNoChIsoGenIso:cicNoChIso,cicNoChIsoNonGenIso:cicNoChIso,cicGenIso:cic,cicNonGenIso:cic \
                  --cut "${eras[$era]}" \
                  --process "*Data*" \
                  --histograms "nvtx>>nvtx(101,0.5,100.5);rho>>rho(20,0,50);mass>>lowmass(560,60,200);leadSigmaIeIe>>leadSigmaIeIe(320,0,3.2e-2);subleadSigmaIeIe>>subleadSigmaIeIe(320,0,3.2e-2);leadR9>>leadR9(110,0,1.1);subleadR9>>subleadR9(110,0,1.1)" \
                  --output output_$era.root;
done;
