...
if [[ -n $rerun  ]] || [[ ! -f $input ]]; then
    ...
    echo "**************************************************************************************************************************"
    subset=$fitname
    ./templates_maker.py --load templates_maker_ZG.json --prepare-data --only-subset $subset --input-dir $treesdir/$input_folder -o $input $verbose $input_opts 2>&1 | tee $input_log
    echo "**************************************************************************************************************************"
fi
