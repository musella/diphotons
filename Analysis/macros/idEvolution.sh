...
src=$1 && shift
www=~/www/exo/spring17/photon_id
label=""
...
# ./idEvolution.py --load idEvolution_isolations.json --load ${src}/${label}eff_areas.json  -O ${wwbase}/offsetCorrectedVars  -i ${src}/output.root/photonDumper/trees  --weight 'weight*(pt>75 && pt<1000)' --wread ${src}/${label}wei.root --pt-corr-var 'pt' --no-pt-corr
...
./idEvolution.py --load idEvolution_isolations.json --load ${src}/${label}eff_areas.json  -O ${wwbase}/correctedVarsVsRho  -i ${src}/output.root/photonDumper/trees  --weight 'weight*(pt>75 && pt<1000)' --wread ${src}/${label}wei.root --pt-corr-var 'pt' --xvar 'rho[1000,5,15]'
