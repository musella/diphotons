...
PFIsolation_struct SuperClusterFootprintRemoval::PFIsolation_worker( reco::SuperClusterRef sc_, edm::Ptr<reco::Vertex> vertexforchargediso, float rotation_phi,
        bool is_recursive_rcone )
{
    ...
    for( unsigned int i = 0; i < pfCandidates->size(); i++ ) {
        ...
        if( type == 1 && vertexforchargediso.isNonnull() ) {
            ...
            float dz = ( pfvertex.z() - phovtx.z() ) - ( ( pfvertex.x() - phovtx.x() ) * vtxmom.x() + ( pfvertex.y() - phovtx.y() ) * vtxmom.y() ) / vtxmom.Perp() *
                       vtxmom.z() / vtxmom.Perp();
            dxy = fabs( dxy );
            dz = fabs( dz );
