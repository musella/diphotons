...
public :
   ...
   float corrChIso(float uncorrChIso, float etaSc);
   float corrNIso(float uncorrNIso, float etaSc);
   float corrGIso(float uncorrGIso, float etaSc);
};

#endif

#ifdef singleGammaPlots_cxx
singleGammaPlots::singleGammaPlots(TTree *tree) : fChain(0) 
{
...
singleGammaPlots::~singleGammaPlots()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t singleGammaPlots::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t singleGammaPlots::LoadTree(Long64_t entry)
{
...
void singleGammaPlots::Init(TTree *tree)
{
...
Bool_t singleGammaPlots::Notify()
{
...
void singleGammaPlots::Show(Long64_t entry)
{
...
Int_t singleGammaPlots::Cut(Long64_t entry)
{
...
#endif // #ifdef singleGammaPlots_cxx
